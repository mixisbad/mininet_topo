...
class MyTopo( Topo ):
    ...
    def __init__( self, maxbw):
        ...
        self.addLink(s1,s3, bw=maxbw)





        self.addLink(s2,server1, bw=maxbw)
        self.addLink(s2,server2, bw=maxbw)

        self.addLink(s2,s3, bw=maxbw)
...
if __name__ == '__main__':
    ...
    client1.cmd('./process_fairness.sh 10.0.0.3 ' + client.IP() + ' ' + controller_ip + ' &')

    client2 = net.get('c2')
    client2.cmd('cd ~/floodlight-qos-beta-master/apps/qos')
    client2.cmd('./process_fairness.sh 10.0.0.4 ' + client.IP() + ' ' + controller_ip + ' &')
